...
export class TeamEffects {
  ...
  loadTeams$: Observable<Action> = this.actions$.pipe(
    ofType<ActionWithPayload<any>>("REQUEST_TEAMS"),
    switchMap((action) => this.teamService.getTeams(action.payload)),
    map((teams: any) => this.teamActions.loadTeamsSuccess(teams))
  );

  @Effect()
  searchTeams$: Observable<Action> = this.actions$.pipe(
    ofType<ActionWithPayload<any>>("SEARCH_TEAMS"),
  ...
  @Effect()
  loadTeamDetails$: Observable<Action> = this.actions$.pipe(
    ofType<ActionWithPayload<any>>("LOAD_TEAM_DETAILS"),
  ...
  @Effect()
  saveTeamDetails$: Observable<Action> = this.actions$.pipe(
    ofType<ActionWithPayload<any>>("SAVE_TEAM_DETAILS"),
    switchMap((action) => {
      ...
      if (action.payload.pictureLogo.url.includes(";base64")) {
        return this.blobStorageService
          .addBase64Image(team.pictureLogo.url, cropperImageName)
          .pipe(
            switchMap((data) => {
              team.pictureLogo = data;
              return this.teamService
                .saveTeamDetails(team)
                .pipe(switchMap((x) => of(team)));
            })
          );
      } else {
        return this.teamService
